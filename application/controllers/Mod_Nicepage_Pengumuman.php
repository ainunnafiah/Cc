	...
	public function index(){          
        ...
        if(isset($_POST['set_pengumuman'])) {
            ...
            if( $this->save_data($get_id,$data) ){ 
                $msg['success'] = 'Berhasil Update Team Departemen';
                if(empty($get_id)) {
                    $msg['success'] = 'Berhasil Menambahkan Team Departemen';
                }
		        
            } else {
                $msg['fail'] = 'Data Tidak Tersimpan';
            } 
            ...
            if( $this->delete_data($id)) {
                $msg['success'] = 'Berhasil Team Departemen'; 
            } else {
                $msg['fail'] = 'Team Departemen Tidak Terhapus';
            }
            $this->session->set_flashdata('alert', $msg);
            redirect($this->uri->segment(1).'/nicepage-pengumuman');
